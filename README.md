...
### 字幕操作（1.2.3）
- 拖入 SRT / VTT / ASS / SSA 文件会附加到当前视频；也可将视频和字幕一起拖入。
- 快速设置和「设置 → 字幕」均可选择外挂或内嵌文本字幕，调整字号、距底部位置及不透明度；显示偏好自动保存。
- `Z`：字幕提前 0.1 秒；`X`：延后 0.1 秒；`Shift + Z`：同步复位。正偏移表示更晚出现，可调范围为 ±60 秒。
- 内嵌文本字幕自动扫描并标记语言。PGS / VobSub 图片字幕与 ASS 复杂特效仍不支持；内嵌字幕读取期间不阻塞视频播放。
